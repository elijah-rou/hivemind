...
## Thematic stack evidence state (2026-08-06)

The durability/safety work is organized as a ten-PR linear stack. The final non-live matrix tested evidence parent `7c798c99171cbc894ddf26e8f9c1e872af143401`, tree `671097ae849bdc7d7e4dc142353a111e70226345`, from `2026-08-06T06:30:04Z` through `06:37:46Z`. All `25 / 25` bounded gates passed with `0` failures, `455s` summed gate duration, and `462s` wall time. The matrix included exact four-thread core seeds `0..9999`, worker seeds `0..999`, and all `28` invoked `run-all.sh --skip-containerd` phases.
...
**Fresh reviewed-stack accepted local verification (2026-08-06T06:30:04Z through 06:37:46Z; tested code commit `7c798c99171cbc894ddf26e8f9c1e872af143401`, tree `671097ae849bdc7d7e4dc142353a111e70226345`; no live infrastructure touched):**
...
- The final matrix passed `25 / 25` bounded gates with `0` failures. Gate durations sum to `455s`; wall time was `462s`. Zig Debug (`37s`) and ReleaseFast (`7s`) passed, followed by all `29` recorded core replays.
- Mutated core sweep: exact seeds `0..9999`, `10,000` tested, `0` failures, four threads, `163.4s` fuzzer elapsed. Worker formatting and all targets passed (`191` library, `3` fuzz utility, `11` main, `1` CLI, `5` integration; containerd integration binary `0`), both recorded worker replays passed, and exact mutated seeds `0..999`, `1,000` tested, `0` failures, four threads, completed in `1.9s`.
- Go API/bench formatting, race tests, and builds; protocol-v6 schema and four-language corpus; active Bash syntax and ShellCheck with sourced-file resolution; Terraform formatting and backend-disabled readonly init/static validation for all four roots; document layout/links; credential scanner self-tests and branch scan; residue self-test and final hygiene all passed.
- `./tests/run-all.sh --skip-containerd` passed all `28` invoked phases in `160s`, including local cleanup, three-replica failover, retained-storage recovery, real-process `/run`, POC teardown ownership, and benchmark cleanup contracts. Containerd component/full-stack remained an explicit skip.
...
- The evidence-bearing docs commit follows this tested parent. Publication records must distinguish `7c798c99171c` / tree `671097ae849b` from the later documentation-only tip.
...
- Historical evidence in this subsection and the changelog remains historical and attests only `bc9f5f63` / tree `78f4c95c`; it is not promoted to the rewrite. Fresh reviewed-stack evidence is recorded separately above against `7c798c99171c...` / tree `671097ae849b...`.
...
- Named scenario `terminal_status_lost_with_session_is_replayed_by_duplicate_start` loses a terminal runtime transition with the old session, reconnects, and proves the leader's duplicate `StartPod` recovers the worker's retained terminal phase instead of leaving control-plane state running forever.
