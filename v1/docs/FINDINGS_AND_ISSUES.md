...
- [ ] verify `ctr images pull` in our direct-`ctr` path honors the AMI's `/etc/containerd/certs.d` `hosts.toml` auth for Hivemind/ECR registries
...
1. **Harden control plane edge** — TLS + auth (gateway token is a first slice; mTLS for agent/replica follows).
2. **Agent auth + shutdown** — pre-shared or mTLS for agent TCP; SIGTERM drain.
3. **App spec** — expand state machine and API payloads without breaking VOPR invariants.
4. **Compaction / snapshots** — bounded replay time at restarts.
5. **Integration** — Thalamus gossip consumer, image pull secrets, traffic split wiring.
