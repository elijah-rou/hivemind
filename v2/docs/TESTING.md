...
| Runtime stop/probe/run failure | Rust unit and named worker simulation cover deterministic stop failure; probe/run faults remain incomplete | A failed stop with runtime status `Running` stays `Stopping`, publishes no `Stopped`, retains mount/CPU/memory/GPU ownership, denies replacement GPU admission, and uses at most three stop attempts; successful retry or terminal status proof permits cleanup | Real process/containerd stop errors, network namespace behavior, and actual mount retention |
