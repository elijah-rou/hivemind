...
The worker simulator models the worker state machine, fail-loud bounded I/O staging, a bounded control-plane stub, simulated runtime, deterministic scheduling, checker-visible resource/state transitions, and bounded bidirectional message delivery. Worker output traverses the simulated network in a later tick; partition, ratio drop/one-shot replay, delay, and path capacity apply in both directions. Delayed partitions preserve queued messages, while explicit session loss discards old-session queues and forces re-registration. Convergence requires observed registration from every worker and an observed terminal status for every generated start command. Runtime fault behaviors remain scenario-specific, and evidence must name the scenario actually exercised. Named scenarios `liveness_probe_two_failures_then_success_resets_counter` and `liveness_probe_three_failures_transition_pod_to_failed` use bounded scripted simulated-runtime outcomes to check probe hysteresis and the third-failure transition. Named scenario `stop_fault_retains_runtime_resources_and_denies_replacement_gpu` injects a deterministic first-stop failure and checks that the still-running runtime retains pod ownership and CPU, memory, and GPU state. Unit regression `failed_stop_retains_running_runtime_ownership_and_capacity` also uses an owned mount marker to prove failed-stop cleanup does not unmount. `Stopped` publication and resource release require terminal runtime status and successful runtime cleanup. Stop signals are attempted at most three times; bounded periodic status-only reconciliation continues after exhaustion. Graceful shutdown remains alive and retains accounting until every runtime becomes terminal and cleanup succeeds. It cannot prove real process signals, host networking/filesystems, `ctr`/containerd behavior, GPU visibility, mounts, or cloud services.
...
| Runtime stop/probe/run failure | Rust unit and named worker simulation cover deterministic stop and scripted liveness-probe outcomes; process-runtime units cover queryable terminal status plus bounded exact HTTP status-line parsing; run faults remain incomplete | A stop stays `Stopping` until runtime status proves terminal and cleanup succeeds, retaining mount/CPU/memory/GPU ownership and denying replacement GPU admission. Liveness probes execute through `Runtime`; success resets the failure count and the third consecutive failure transitions the pod to `Failed`. Process probes accept only an exact well-formed HTTP 200 status, with bounded connect/read/write deadlines, status line, and request target. Stop signals use command grace and at most three attempts, followed by bounded status-only reconciliation. Graceful shutdown does not exit while runtime ownership remains unverified. | Real containerd signal/error behavior and task-network-namespace probing require the privileged component gate; actual JuiceFS mount retention remains unverified |
