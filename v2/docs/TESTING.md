...
The fresh non-live matrix ran from `2026-07-27T00:38:20Z` through `00:46:34Z` against PR10 evidence parent `b08e7081bf8ad894f2b97e617079a2e333ca6865`, tree `3a896d9009ba97cac9f3ed1fd4093fd05e89ad77`. It passed `25 / 25` bounded gates with `0` failures, `486s` summed gate duration, and `494s` wall time. The final evidence/governance docs commit is a descendant and is recorded separately from this tested parent.

Core Debug/ReleaseFast, 29 regression replays, and exact mutated seeds `0..9999` passed; the four-thread core sweep tested `10,000` seeds with `0` failures in `170.4s`. Worker formatting/all-targets, both recorded regression replays, and exact mutated seeds `0..999` passed; the four-thread worker sweep tested `1,000` seeds with `0` failures in `1.9s`. The worker all-target result was `186` library, `3` fuzz utility, `7` main, `5` integration, and `0` containerd-integration tests. The bounded stderr-capture self-test proved that an stderr-only JSON summary is captured and that a wrong expected count fails closed.

Go API/bench format, race, and build; protocol-v6 schema/four-language corpus; active Bash syntax and default ShellCheck; all four offline Terraform roots; changed-document layout/links; credential scanner positive/negative/redaction fixtures and branch scan; local-residue signatures; and final cleanliness passed. `./tests/run-all.sh --skip-containerd` passed all `26` invoked phases in `184s`. Final inventories found zero branch-owned processes, zero relevant listener delta, zero port-lock delta, zero generated-artifact delta, a clean tracked/index state, and no `v1` change.

All failed attempts remain separate from acceptance. Preserved run `20260726T233943Z-b08e7081bf8a` exposed stdout-only capture of a successful stderr core summary. Full-from-gate-1 run `20260727T002042Z-b08e7081bf8a` stopped on a transient two-second systemd fixture deadline under aggregate load; its immediate bounded focused rerun passed. Full-from-gate-1 run `20260727T002955Z-b08e7081bf8a` stopped on an untracked checker that included intentionally frozen/legacy relocation links; the checker was narrowed to branch-changed Markdown. Accepted run `20260727T003820Z-b08e7081bf8a` started again at gate 1 after those support corrections. No failed run was resumed or promoted.
