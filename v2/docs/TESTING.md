...
| Runtime stop/probe/run failure | Rust unit and named worker simulation cover deterministic stop failure; process-runtime units cover queryable terminal status; probe/run faults remain incomplete | A stop stays `Stopping` until runtime status proves terminal and cleanup succeeds, retaining mount/CPU/memory/GPU ownership and denying replacement GPU admission. Stop signals use command grace and at most three attempts, followed by bounded status-only reconciliation. Graceful shutdown does not exit while runtime ownership remains unverified. | Real containerd signal/error behavior requires the privileged component gate; network namespace behavior and actual JuiceFS mount retention remain unverified |
