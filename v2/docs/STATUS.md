...
The durability/safety work is organized as a ten-PR linear stack. The final non-live matrix tested evidence parent `3890ac40615b60e7775637a2f59d042c2d893c98`, tree `822a5c77c1e446058ca701e61c238bd132a246e0`, from `2026-08-06T05:45:20Z` through `05:53:10Z`. All `25 / 25` bounded gates passed with `0` failures, `463s` summed gate duration, and `470s` wall time. The matrix included exact four-thread core seeds `0..9999`, worker seeds `0..999`, and all `28` invoked `run-all.sh --skip-containerd` phases.
...
**Fresh reviewed-stack accepted local verification (2026-08-06T05:45:20Z through 05:53:10Z; tested code commit `3890ac40615b60e7775637a2f59d042c2d893c98`, tree `822a5c77c1e446058ca701e61c238bd132a246e0`; no live infrastructure touched):**
...
- The final matrix passed `25 / 25` bounded gates with `0` failures. Gate durations sum to `463s`; wall time was `470s`. Zig Debug (`37s`) and ReleaseFast (`7s`) passed, followed by all `29` recorded core replays.
- Mutated core sweep: exact seeds `0..9999`, `10,000` tested, `0` failures, four threads, `158.8s` fuzzer elapsed. Worker formatting and all targets passed (`190` library, `3` fuzz utility, `11` main, `1` CLI, `5` integration; containerd integration binary `0`), both recorded worker replays passed, and exact mutated seeds `0..999`, `1,000` tested, `0` failures, four threads, completed in `1.9s`.
...
- `./tests/run-all.sh --skip-containerd` passed all `28` invoked phases in `173s`, including local cleanup, three-replica failover, retained-storage recovery, real-process `/run`, POC teardown ownership, and benchmark cleanup contracts. Containerd component/full-stack remained an explicit skip.
...
- The evidence-bearing docs commit follows this tested parent. Publication records must distinguish `3890ac40615b` / tree `822a5c77c1e4` from the later documentation-only tip.
...
- Historical evidence in this subsection and the changelog remains historical and attests only `bc9f5f63` / tree `78f4c95c`; it is not promoted to the rewrite. Fresh reviewed-stack evidence is recorded separately above against `3890ac40615b...` / tree `822a5c77c1e...`.
