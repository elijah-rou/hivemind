...
The durability/safety work is organized as a ten-PR linear stack. The final non-live matrix tested evidence parent `73b7eca987731fd325b06f6bab419cd1b347668a`, tree `99d635554429d5c2e664c179478ebab1338a8f6f`, from `2026-08-06T06:13:12Z` through `06:21:39Z`. All `25 / 25` bounded gates passed with `0` failures, `498s` summed gate duration, and `507s` wall time. The matrix included exact four-thread core seeds `0..9999`, worker seeds `0..999`, and all `28` invoked `run-all.sh --skip-containerd` phases.
...
**Fresh reviewed-stack accepted local verification (2026-08-06T06:13:12Z through 06:21:39Z; tested code commit `73b7eca987731fd325b06f6bab419cd1b347668a`, tree `99d635554429d5c2e664c179478ebab1338a8f6f`; no live infrastructure touched):**
...
- The final matrix passed `25 / 25` bounded gates with `0` failures. Gate durations sum to `498s`; wall time was `507s`. Zig Debug (`37s`) and ReleaseFast (`7s`) passed, followed by all `29` recorded core replays.
- Mutated core sweep: exact seeds `0..9999`, `10,000` tested, `0` failures, four threads, `161.9s` fuzzer elapsed. Worker formatting and all targets passed (`191` library, `3` fuzz utility, `11` main, `1` CLI, `5` integration; containerd integration binary `0`), both recorded worker replays passed, and exact mutated seeds `0..999`, `1,000` tested, `0` failures, four threads, completed in `1.9s`.
...
- `./tests/run-all.sh --skip-containerd` passed all `28` invoked phases in `184s`, including local cleanup, three-replica failover, retained-storage recovery, real-process `/run`, POC teardown ownership, and benchmark cleanup contracts. Containerd component/full-stack remained an explicit skip.
...
- The evidence-bearing docs commit follows this tested parent. Publication records must distinguish `73b7eca98773` / tree `99d635554429` from the later documentation-only tip.
...
- Historical evidence in this subsection and the changelog remains historical and attests only `bc9f5f63` / tree `78f4c95c`; it is not promoted to the rewrite. Fresh reviewed-stack evidence is recorded separately above against `73b7eca98773...` / tree `99d635554429...`.
