...
6. Opening an existing `journal.bin` that is not exactly the expected layout size fails closed (no silent empty re-init of truncated/partial journals). New journals are created exclusively (`O_EXCL`) and the parent directory is fsynced. FileDisk layout is version 2: explicit little-endian LogEntry codec with checked tag-first Command decoding (never native `@sizeOf(LogEntry)`/`asBytes`). Legacy v1 is rejected. This POC does not support mixed-version peer clusters or in-place legacy-v1 journal upgrades. Operators must stop the full cluster and restart v2 with fresh data directories or legacy data explicitly archived/replaced out of band. No rolling migration or incarnation protocol is implemented or claimed. Fail-closed 1024-op retained-log lifetime cap remains.
