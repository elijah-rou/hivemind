...
- Bench transfer artifacts use an internally generated AWS-account-scoped 128-bit run identity, content-addressed object keys, and an ownership-installed exit trap. A conditional per-invocation marker claim prevents us-east-1 already-owned success or same-token races from becoming ownership. AWS calls use GNU `timeout` with TERM then bounded KILL for the full process group. Creation and upload fail closed; cleanup revalidates the exact marker before removing only the owned run prefix and bucket unless explicit keep mode is enabled.
