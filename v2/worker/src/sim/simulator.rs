...
mod tests {
    ...
    #[test]
    fn lost_registration_requires_retry_after_healing() {
        ...
        sim.retry_agent_registration(0);
        sim.run(2);
    ...
    fn outbound_partition_before_pod_status_delays_delivery_until_healing() {
        ...
        assert_eq!(
            ...
            vec![PodStatusReport::Running],
            "status writes attempted during the partition are lost, while the post-heal state is observed"
        );
        assert_eq!(
            received_count(&sim, |message| matches!(
                message,
        ...
        assert!(sim.network.stats.worker_failed_partition >= 2);
    }
    ...
    #[test]
    fn run_response_written_after_partition_is_lost() {
